...
class FeatureFusionLayer(nn.Module):
    ...
    def forward_post(self, src1, src2,
        ...
        src3 = self.multihead_attn1(query=self.with_pos_embed(src1, pos_src1),
                                     key=self.with_pos_embed(src2, pos_src2),
                                     value=src2, attn_mask=src2_mask,
                                     key_padding_mask=src2_key_padding_mask)[0]
        src3 = src1 + self.dropout12(src3)
        src3 = self.norm12(src3)
        src31 = self.linear12(self.dropout1(self.activation1(self.linear11(src3))))
        src3 = src3 + self.dropout13(src31)
        src3 = self.norm13(src3)


        src12 = self.multihead_attn1(query=self.with_pos_embed(src1, pos_src1),
                                     key=self.with_pos_embed(src3, pos_src1),
                                     value=src3, attn_mask=src1_mask,
